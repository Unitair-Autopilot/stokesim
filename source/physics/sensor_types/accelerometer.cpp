/** @file
Implementation of accelerometer.h
*/
...
Accelerometer::Accelerometer(YAML::Node const& config, Craft const* container) :
...
void Accelerometer::update_states(Cmd const& cmd, Escal dt) {
...
void Accelerometer::update_measurements(Cmd const& cmd) {
    // Find acceleration at mounting point on body (apply transport theorem twice)
    Body const& body = craft->body;
    Evec3 accel = rotmat_inv*(body.veldot_inr +
                              body.aveldot_inr.cross(pos) +
                              body.avel_inr.cross(body.avel_inr.cross(pos)));
    Evec3 noisy_accel = accel + noise + bias;
    measurements.at("x") = noisy_accel(0);
    measurements.at("y") = noisy_accel(1);
    measurements.at("z") = noisy_accel(2);
}
